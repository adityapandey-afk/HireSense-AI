...
API_URL = os.getenv("API_URL", "https://hiresense-backend-oqda.onrender.com")
...
def login_page():
    st.markdown(
        "<h1 style='text-align: center; margin-top: 50px;' class='text-glow'>🧠 HireSense AI</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='text-align: center; color: #9ca3af;'>Next-gen recruiting & resume ranking platform</p>",
        unsafe_allow_html=True,
    )
    ...
    with col2:
        ...
        with tab2:
            ...
            reg_password = st.text_input(
                "Password (min 8 chars)", type="password", key="reg_pass"
            )
...
def draw_sidebar():
    st.sidebar.markdown(
        "<h2 style='text-align: center; color: #6366f1; margin-top: 20px; font-weight: 800;'>🧠 HireSense AI</h2>",
        unsafe_allow_html=True,
    )
    st.sidebar.markdown(
    ...
    def make_nav_button(label, key_name):
        if st.sidebar.button(
            ...
            type=(
                "primary" if st.session_state["active_tab"] == key_name else "secondary"
            ),
        ):
    ...
    if st.session_state["role"] == "recruiter":
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                with c1:
                    st.markdown(
                        f"<div class='glass-card' style='text-align: center;'><h3>💼 Jobs Posted</h3><h2 style='color:#6366f1; margin:0;'>{stats['jobs_posted']}</h2></div>",
                        unsafe_allow_html=True,
                    )
                with c2:
                    st.markdown(
                        f"<div class='glass-card' style='text-align: center;'><h3>👥 Total Applications</h3><h2 style='color:#a855f7; margin:0;'>{stats['total_applications']}</h2></div>",
                        unsafe_allow_html=True,
                    )
                with c3:
                    st.markdown(
                        f"<div class='glass-card' style='text-align: center;'><h3>✅ Shortlisted</h3><h2 style='color:#10b981; margin:0;'>{stats['shortlisted']}</h2></div>",
                        unsafe_allow_html=True,
                    )
                with c4:
                    st.markdown(
                        f"<div class='glass-card' style='text-align: center;'><h3>📈 Avg Match Score</h3><h2 style='color:#0ea5e9; margin:0;'>{stats['average_match_score']}%</h2></div>",
                        unsafe_allow_html=True,
                    )
            else:
        ...
        with c1:
            st.markdown(
                "<div class='glass-card' style='text-align: center;'><h4>Resume Parser</h4><p style='color: #10b981; font-weight: 700; margin: 0;'>Active</p><span style='font-size:0.8rem; color:#9ca3af;'>Extracts contact info & skill vectors</span></div>",
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                "<div class='glass-card' style='text-align: center;'><h4>JD Alignment</h4><p style='color: #10b981; font-weight: 700; margin: 0;'>Active</p><span style='font-size:0.8rem; color:#9ca3af;'>SentenceTransformer semantic similarity</span></div>",
                unsafe_allow_html=True,
            )
        with c3:
            st.markdown(
                "<div class='glass-card' style='text-align: center;'><h4>GitHub Verification</h4><p style='color: #10b981; font-weight: 700; margin: 0;'>Active</p><span style='font-size:0.8rem; color:#9ca3af;'>Analyzes repositories & languages</span></div>",
                unsafe_allow_html=True,
            )
        with c4:
            st.markdown(
                "<div class='glass-card' style='text-align: center;'><h4>Dynamic Scoring</h4><p style='color: #10b981; font-weight: 700; margin: 0;'>Active</p><span style='font-size:0.8rem; color:#9ca3af;'>Dynamic weights based on profile</span></div>",
                unsafe_allow_html=True,
            )
...
def page_explore_jobs():
    st.markdown(
        "<h2 class='text-glow'>💼 Explore Job Opportunities</h2>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='color: #9ca3af;'>Browse openings, review required skills, and match your resume.</p>",
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            for job in jobs:
                ...
                with col1:
                    if st.button(
                        "Apply & Match", key=f"apply_{job['id']}", type="primary"
                    ):
...
def page_my_applications():
    st.markdown(
        "<h2 class='text-glow'>📋 My Applications & Status</h2>", unsafe_allow_html=True
    )
    st.markdown(
        "<p style='color: #9ca3af;'>Track your match scores and status decisions from recruiters.</p>",
        unsafe_allow_html=True,
    )
...
def page_post_job():
    st.markdown(
        "<h2 class='text-glow'>📋 Post a New Job opening</h2>", unsafe_allow_html=True
    )
    st.markdown(
        "<p style='color: #9ca3af;'>Publish requirements for automated candidate ranking & matching.</p>",
        unsafe_allow_html=True,
    )

    with st.form("post_job_form", clear_on_submit=True):
        title = st.text_input(
            "Job Title *", placeholder="e.g. Lead Full-Stack Engineer"
        )
        ...
        if submit:
            if not all([title, company, description, required_skills]):
                ...
                try:
                    ...
                    if resp.status_code == 201:
                        st.success(
                            f"🎉 Job successfully published! Job ID: {resp.json()['job_id']}"
                        )
                    else:
...
def page_manage_jobs():
    st.markdown(
        "<h2 class='text-glow'>⚙️ Manage Jobs & Applicants</h2>", unsafe_allow_html=True
    )
    st.markdown(
        "<p style='color: #9ca3af;'>View applicant rankings, skill gaps, and send shortlist notifications.</p>",
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            selected_option = st.selectbox(
                "Select Job Posting to Audit", list(job_opts.keys())
            )
            ...
            app_resp = requests.get(
                f"{API_URL}/recruiter/jobs/{selected_job['id']}/applications",
                headers=auth_header(),
            )
            if app_resp.status_code == 200:
                ...
                for idx, app in enumerate(applicants):
                    ...
                    # Card for applicant
                    with st.expander(
                        f"🏅 {app['candidate_name']} ── Score: {app['match_score']}% ── status: {status.upper()}",
                        expanded=idx == 0,
                    ):
                        st.markdown(
                            f"""
                        ...
                        if app["skill_gaps"]:
                            st.markdown(
                                f"<div style='margin-top: 10px; color:#ef4444;'>⚠️ <strong>Missing skills:</strong> {app['skill_gaps']}</div>",
                                unsafe_allow_html=True,
                            )
                        else:
                            st.markdown(
                                "<div style='margin-top: 10px; color:#10b981;'>✅ <strong>All required skills met!</strong></div>",
                                unsafe_allow_html=True,
                            )
                        ...
                        email_notify = st.checkbox(
                            ...
                            value=True,
                            key=f"notify_check_{app['match_id']}",
                        )
...
def update_status_api(
    match_id, status, candidate_name, candidate_email, score, send_mail
):
    try:
        resp = requests.post(
            f"{API_URL}/recruiter/applications/{match_id}/status",
        ...
        if resp.status_code == 200:
            ...
            if send_mail:
                ...
                if mail_resp.status_code == 200:
                    ...
                    st.warning(
                        "Application saved, but notification email failed to send (check server configurations)."
                    )
...
def page_analyze_candidate():
    st.markdown(
        "<h2 class='text-glow'>🔍 Resume Matching & Rank Analytics</h2>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='color: #9ca3af;'>Parse resume files, match against standard or specific JDs, and inspect GitHub metrics.</p>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        github_username = st.text_input(
            "GitHub Username (optional)", placeholder="e.g. octocat"
        )
    with col2:
        if job_id:
            st.text_area(
                "Job Description (Auto-loaded from job posting)",
                value="Job details pre-loaded. Description is managed by backend.",
                height=150,
                disabled=True,
            )
            ...
            jd_text = st.text_area(
                "Job Description *",
                height=150,
                placeholder="Paste the details of the job requirements here...",
            )
    ...
    if st.button("Run Profile Analysis 🚀", type="primary", use_container_width=True):
        if not uploaded_file or (not job_id and not jd_text.strip()):
            st.warning(
                "Resume file and Job Description (or Selected Job) are required."
            )
        else:
            with st.spinner("Analyzing profile & fetching metrics..."):
                try:
                    ...
                    st.error(
                        "Request timed out. GitHub lookup took too long — try matching without providing a GitHub username."
                    )
                except Exception as e:
                    st.error(f"Error: {e}")
    ...
    if result:
        ...
        st.markdown(
            f"### 📊 Scorecard for **{result['candidate_name']}**",
            unsafe_allow_html=True,
        )
        ...
        with col1:
            st.markdown(
                f"<div class='glass-card' style='text-align: center; border-bottom: 4px solid #10b981;'><h4>Final Score</h4><h1 style='color: #10b981;'>{ranking['final_score']}%</h1></div>",
                unsafe_allow_html=True,
            )
        with col2:
            st.markdown(
                f"<div class='glass-card' style='text-align: center; border-bottom: 4px solid #a855f7;'><h4>Grade</h4><h1 style='color: #a855f7;'>{ranking['grade']}</h1></div>",
                unsafe_allow_html=True,
            )
        with col3:
            st.markdown(
                f"<div class='glass-card' style='text-align: center; border-bottom: 4px solid #0ea5e9;'><h4>Recommendation</h4><h3 style='color: #0ea5e9; margin-top:12px;'>{ranking['recommendation']}</h3></div>",
                unsafe_allow_html=True,
            )
        ...
        gauge_color = (
            "#10b981"
            if score_val >= 70
            else ("#f59e0b" if score_val >= 40 else "#ef4444")
        )
        fig = go.Figure(
            go.Indicator(
                mode="gauge+number",
        ...
        with col1:
            ...
            if result["skills"]:
                skills_html = "".join(
                    [
                        f"<span class='badge badge-pending' style='margin: 4px; font-size: 0.9rem;'>{s}</span>"
                        for s in result["skills"]
                    ]
                )
                st.markdown(
                    f"<div style='margin-bottom: 20px;'>{skills_html}</div>",
                    unsafe_allow_html=True,
                )
            else:
        ...
        with col2:
            ...
            if g.get("username"):
                ...
                if g["top_repos"]:
                    st.markdown(
                        "<p style='margin-top: 10px; font-weight:700;'>Recent Active Repos:</p>",
                        unsafe_allow_html=True,
                    )
        ...
        if st.session_state["role"] == "recruiter" and not job_id:
            ...
            candidate_email = st.text_input(
                "Candidate Notification Email Address", value=result["candidate_email"]
            )

            col1, col2 = st.columns(2)
            with col1:
                if st.button(
                    "✅ Shortlist Candidate", type="primary", use_container_width=True
                ):
                    notify_sandbox_decision(
                        candidate_email,
...
def notify_sandbox_decision(email, name, decision, score):
    ...
    try:
        ...
        if resp.status_code == 200:
            st.success(
                f"Email sent successfully notifying {name} of {decision.upper()} status!"
            )
        else:
